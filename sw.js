...
self.addEventListener('install',event=>{
  event.waitUntil(
    caches.open(CACHE).then(async cache=>{
      ...
      const shell=await firstCachedShell(cache);
      if(!shell)throw new Error('Playable shell failed to cache');
...
async function firstCachedShell(cacheStorage=caches){
  for(const candidate of SHELL_CANDIDATES){
    const response=await cacheStorage.match(candidate);
  ...
  if(event.request.mode==='navigate'){
    event.respondWith(
      Promise.resolve(event.preloadResponse)
        ...
        .catch(async()=>{
          const cached=await firstCachedShell();
